...
**Next task on the queue:** **Task 14d — Sprite Display Simplification.** Simplify the sprite display to a single 80×80 asset, remove the sprite from the Stats screen entirely, and reflow the Interact screen to fit the larger sprite. Must land before Task 13a (Sprite Animation). Full design in `DEV_ROADMAP.md` → Task 14d section. Branch: `task/14d-sprite-simplification`.
...
**Just completed:** Task 14c — Gameplay Balance Tuning. Rebalanced the five decay rates so the three fatal stats (hunger, energy, happiness) each reach a fatal level in ~10–11 min and within 2× of each other; cleanliness and sickness stay slightly slower as secondary, non-fatal concerns. Every stat now moves 1 point per interval, so each interval reads directly as "1 point every N ms." Added a 2000 ms `millis()` cooldown to `ImuManager::wasShaken()` (the gate lives in `update()`; `wasShaken()` stays a `const` query) so one shake gesture triggers `play()` once instead of every frame, and cut `Pet::play()`'s energy cost from 20 to 5. `time_manager.cpp` keeps **two** complete value sets — the shipped set and a quick-testing set (~1 min to fatal) — selected by an in-file `#define FAST_TEST` toggle at the top of the file (left commented for the shipped build; no `platformio.ini` change, no runtime difficulty setting). Both sets are kept on purpose as a worked balancing example. Verified the fast build on device. Branch: `task/14c-balance-tuning`.
