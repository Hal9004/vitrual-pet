...
**Next task on the queue:** **Task 21 — Curriculum Scaffolding Refactor.** Add `#define ENABLE_*` flags so the codebase can be configured for any session's day-start state (ENABLE_ACTION_MENU / ENABLE_IMU_PLAY / ENABLE_SOUND / ENABLE_PERSISTENCE / ENABLE_MULTISCREEN / ENABLE_MOOD_SPRITES). Every flag combination must compile + flash cleanly. Full design in `DEV_ROADMAP.md` → Task 21 section.

**Then: Task 22 — Doc Sweep**, after which work moves to the `virtual-pet-learning-lab` repo (Phases 2–4 of the realignment). See `DEV_ROADMAP.md` for both.
