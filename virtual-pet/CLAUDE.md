...
**Next task on the queue:** **Task 13a — Sprite Animation.** Cycle a multi-frame 80×80 sprite at runtime using `millis()` (no blocking), drawing through an off-screen `M5Canvas` to kill flicker, and implement `lib/Display/animation_manager.h/.cpp`. The asset pipeline and call sites already index frame `[0]`, so the size is locked. Full design in `DEV_ROADMAP.md` → Task 13a section. Branch: `task/13a-sprite-animation`.
...
**Just completed:** Task 14d — Sprite Display Simplification. Collapsed the project to a single `sprite_80x80_test` asset drawn on every screen. Removed the sprite from the Stats screen (now a pure data view) and the now-unused `PET_FACE_ZONE`. Rather than "compressing the Interact free space," aligned the Interact screen's face (`INTERACT_FACE_CENTER_Y = 110`) and mood (`INTERACT_MOOD_Y = 155`) to the Main screen's positions and moved `MAIN_NAV_ZONE` to a shared y=220 (= `MENU_ZONE`), so the sprite, mood, and bottom bar do not appear to move when switching screens; the contextual stat bar dropped to y=174–204. Folded in a cleanup: removed the post-action green feedback text and the `delay(1000)` in `ActionMenu::confirmAction()` (the delay broke the non-blocking-loop rule) and deleted the now-unused `DisplayManager::showActionFeedback()`. Deleted the 48×48/64×64/newpiskel2 `.h`/`.c` assets and repointed `main.cpp`'s `SPRITE_TEST` harness to the 80×80 asset. The `80x80_test` name is kept as a dev placeholder. Branch: `task/14d-sprite-simplification`.
