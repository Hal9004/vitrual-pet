...
**Next task on the queue:** **Task 22 — Doc Sweep** (in progress on `task/22-doc-sweep`). With the code in final form, bring the docs into line: rewrite `COURSE_CHECKLIST.md` against the 10-session arc; purge `IDEAS.md` of items now in core scope (IMU-driven movement and mood sprites have shipped); audit `USEFUL_NOTES.md` for accuracy; mark Task 21 done in `DEV_ROADMAP.md` / `CURRICULUM_REALIGNMENT.md`; and fix two wordings Task 21 surfaced — the session table's "Session 6 adds multi-screen" → "adds the Stats screen", and "every flag combination must compile" → the cumulative-staircase guarantee (the 2^6 combos are NOT all valid: `ENABLE_PERSISTENCE` requires `ENABLE_ACTION_MENU`). The final step of Task 22 is to update THIS pointer to "Define Session 1 lesson plan". Full scope in `DEV_ROADMAP.md` → Task 22 section.
