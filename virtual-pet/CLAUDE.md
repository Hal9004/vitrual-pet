...
**Next task on the queue:** **Task 13b — Tilt-Reactive Sprite Movement (demo).** A small, optional demo (designed to be easy to toggle on/off) where the pet sprite slides around the screen based on live accelerometer tilt. Add a tiny `TiltMotion` helper (`lib/Display/tilt_motion.{h,cpp}`) that maps tilt → a low-pass-smoothed, clamped (x, y) offset; `DisplayManager::drawPetSprite()` applies the offset; `main.cpp` owns the helper, feeds it `imu.getAccelX/Y()` (currently unused), and gates it behind one `TILT_MOVEMENT_ENABLED` flag. Always-on, **no Pet state-machine change** — kept deliberately small. Full design captured in the plan file (`~/.claude/plans/validated-questing-shore.md`). To be built in its own session on branch `task/13b-tilt-movement`.

**Then: Task 20 — Mood Sprite System.** Add a `MoodSprite` enum (NEUTRAL / HAPPY / UNWELL / HUNGRY), a `Pet::computeMood()` that maps stats → mood with prioritised thresholds, and 4 sprite assets in `lib/Display/sprites/`. The animation infrastructure from Task 13a (`AnimationManager`, the M5Canvas, the unused-but-reserved `moodIndex` parameter in `drawPetSprite()`) is the hook point. Mood threshold values are an open question tracked in `CURRICULUM_REALIGNMENT.md`. Full design in `DEV_ROADMAP.md` → Task 20 section.
...
**Just completed:** Task 13a — Sprite Animation. Regenerated `sprite_80x80_test` as a 2-frame asset (synthetic ~2px bounce placeholder; real Piskel art to follow) so the converter's `FRAME_COUNT > 1` path is exercised. Implemented `lib/Display/animation_manager.h/.cpp` — a standalone `AnimationManager` that cycles frames with the non-blocking `millis()` pattern (mirrors `TimerManager`): `update()`, `getCurrentFrame()`, `reset()`, `FRAME_DURATION_MS = 200` (5 fps), with a `frameCount <= 1` guard. Routed all `DisplayManager` drawing through a 135×240 16-bit `M5Canvas` double-buffer (`pushCanvas()` blits once per frame) — `setColorDepth(16)` + `createSprite()` in `init()`; verified on-device that the pre-byte-swapped sprite colours render correctly through the canvas (no `setSwapBytes` needed; a commented knob is left in `init()`). Wired `AnimationManager` into `DisplayManager` (`getCurrentFrame()` indexes both sprite draw sites; `reset()` on screen change) and **removed the 5s `STATUS_UPDATE_INTERVAL` throttle** + the Interact fast-path, so every frame redraws flicker-free and stat changes are instant. Dropped the now-dead `currentActionIndex` end-to-end (incl. `main.cpp`); left `ActionMenu::getCurrentActionIndex()` in place (general accessor, not simplification-pass scope). Documented all of it in `SPRITE_GUIDE.md` Part 6. Branch: `task/13a-sprite-animation`.
